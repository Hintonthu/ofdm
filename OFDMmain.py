...
L1_1 = tf.layers.batch_normalization(L1_1)
...
L2_1 = tf.layers.batch_normalization(L2_1)
...
L3_1 = tf.layers.batch_normalization(L3_1)
...
L4_1 = tf.layers.batch_normalization(L4_1)
...
encoded_symbol_normalizing = tf.sqrt(tf.reduce_mean(tf.abs(encoded_symbol_iffted)))
encoded_symbol_original_r = (1/np.sqrt(2))*tf.div(tf.real(encoded_symbol_iffted), encoded_symbol_normalizing)
encoded_symbol_original_i = (1/np.sqrt(2))*tf.div(tf.imag(encoded_symbol_iffted), encoded_symbol_normalizing)
...
cost = tf.reduce_mean(tf.square(hypothesis-Y)) + 0.0001*tf.reduce_mean(peak_power_symbol)
...
figure()
for SNR_range in np.arange(0,1,5):
    # train my model
    ...
    for iterN in range(len(lin_space)):
        ...
        if lin_space[iterN] == SNR_range:
            training_epochs = 300000#100+SNR_range*30000
            for epoch in range(training_epochs):
                ...
                corruption_batch = tf.complex(corruption_r, corruption_i)
    ...
    for iterN in range(len(lin_space)):
        ...
        corruption_test_batch = tf.complex(corruption_r, corruption_i)
        ...
        err_rate.append(SER)
    np.savetxt("./result/OFDM_SER_trained_at_{0}dB SNR_L4".format(SNR_range), err_rate)
